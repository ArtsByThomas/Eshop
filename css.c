...
void parse_css_declaration(const char** p, ComputedStyle* style) {
    ...
    if (str_eq(prop, "color")) {
    ...
    } else if (str_eq(prop, "background-color") || str_eq(prop, "background")) {
        style->has_bg = 1;
    ...
    } else if (str_eq(prop, "margin")) {
        style->has_margin = 1;
        style->margin = parse_css_length(p);
    } else if (str_eq(prop, "padding")) {
        style->has_padding = 1;
        style->padding = parse_css_length(p);
    } else if (str_eq(prop, "display")) {
        style->has_display = 1;
        skip_spaces(p);
        if (str_starts_with(*p, "inline")) { style->is_inline = 1; *p += 6; }
        else if (str_starts_with(*p, "block")) { style->is_inline = 0; *p += 5; }
        else if (str_starts_with(*p, "none")) { style->is_inline = -1; *p += 4; }
    } else if (str_eq(prop, "font-size")) {
    ...
    } else if (str_eq(prop, "float")) {
        style->has_float = 1;
        skip_spaces(p);
        if (str_starts_with(*p, "left")) { style->float_side = 1; *p += 4; }
        else if (str_starts_with(*p, "right")) { style->float_side = 2; *p += 5; }
        else { style->float_side = 0; while (is_alpha_c(**p)) (*p)++; } 
    } 
    else if (str_eq(prop, "text-align")) {
        style->has_text_align = 1;
        skip_spaces(p);
        if (str_starts_with(*p, "center")) { style->text_align = 1; *p += 6; }
        else if (str_starts_with(*p, "right")) { style->text_align = 2; *p += 5; }
        else { style->text_align = 0; while (is_alpha_c(**p)) (*p)++; }
    } else if (str_eq(prop, "border")) {
        style->has_border = 1;
        ...
        skip_spaces(p);
        style->border_color = parse_css_color(p);
    } else if (str_eq(prop, "border-radius")) { 
